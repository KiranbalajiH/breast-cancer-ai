...
def evaluate_model_on_test(model, X_test, y_test, classes, test_paths=None):
    ...
    # Exact number of incorrect predictions
    incorrect_count = int(np.sum(pred_classes != y_test))
    
    # Detailed misclassification examples
    misclassifications = []
...
def main():
    ...
    for name, config in candidates.items():
        ...
        # Evaluate
        eval_metrics = evaluate_model_on_test(model, X_test_prep, y_test, classes, test_paths=paths_test)
        ...
        print(f"  Total Incorrect Predictions: {eval_metrics['incorrect_count']}")
        ...
        for mc in eval_metrics['misclassifications']:
            print(f"    - File: {mc['filename']} | True: {mc['true_class']} | Predicted: {mc['predicted_class']} (Conf: {mc['confidence']:.4f})")
    ...
    if os.path.exists(v1_model_path):
        ...
        v1_metrics = evaluate_model_on_test(v1_model, X_test_mnv2, y_test, classes, test_paths=paths_test)
        ...
        print(f"  Total Incorrect Predictions: {v1_metrics['incorrect_count']}")
